...
const run = async () => {
    ...
    var tableData = [];
    while (dataControl) {
        console.log('------------ Verileri getirmeye başladık');
    ...
    console.log(JSON.stringify(tableData.filter(x => x.mainCategory === 'Süpermarket')));
    client.set("categories", JSON.stringify(tableData.filter(x => x.mainCategory === 'Süpermarket')));
...
const onLogin = async (page) => {
    await page.goto('https://partner.trendyol.com/account/login', { waitUntil: 'domcontentloaded' });
